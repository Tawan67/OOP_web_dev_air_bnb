...
def add_member_and_payment_method(control_system):
    ...
    new_pay_med = PaymentMethod(bank_id="1234", user=new_member, balance=1000)
...
def add_accommodation(control_system):
    ...
    reset.reset_increament()
    new_house = House("test_house", "location", "description", 0)
    ...
    new_room = Room(room_id="1", room_floor=1, price=100, hotel_address="location_hotel", hotel_name="test_hotel")
...
def make_booking(control_system):
    ...
    control_system.create_booking(
        accom=control_system.get_accommodation_list[1],
        date=BookedDate(datetime.now(), datetime.now() + timedelta(days=5)),
        guess=5,
        member=control_system.get_member_list[0]
    )
...
async def purchase(req, booking_id: int):
    control_system = req.app.state.control_system
    result_booking = control_system.search_booking_by_id(booking_id)

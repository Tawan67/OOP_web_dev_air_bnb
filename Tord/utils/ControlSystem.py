...
class ControlSystem:
    ...
    def process_payment(
        ...
        if self.check_accom_available(process_booking):
            ...
            add_status = process_booking.get_accommodation.add_booked_date(
                booked_date)
            if add_status == "Error":
...
"""
    ...
    def get_total_price_to_show(self, booking_id, out_int=False, use_cou=True):
        try:
            booking_item = self.search_booking_by_id(booking_id)
            accom = booking_item.get_accommodation
            coupon = booking_item.get_cou
            # price = accom.get_price
    ...
    def get_all_price(self, booking_id, out_int=False):  # true if want int
        try:
            book_item = self.search_booking_by_id(booking_id)
            coupon = book_item.get_cou
            total_price_with_fee = self.get_fee(
                booking_id, out_int=True)+self.get_total_price_to_show(booking_id, out_int=True)
            if coupon != None:
                total_price_with_fee = coupon.use(total_price_with_fee)
            if out_int:
                    ...
                    Div(
                        Div(
                            ...
                            Div(
                                ...
                                P(f"Rating : {self.get_av_rating(book_id)}⭐",
                                  style="margin: 0; padding-left: 10px;"),
                        ...
                        Div(
                            Table(
                                Tr(
                                    Td(f"${self.get_price_per_night(book_id)} x {self.get_night(book_id)} nights",
                                       style="background-color:white;color:black;"),
                                    Td(f"${self.get_total_price_to_show(book_id)}",
                                       style="background-color:white;color:black;"),
                                ),
                                Tr(
                                    Td("fee", style="background-color:white;color:black;"),
                                    Td("$"+self.get_fee(book_id),
                                       style="background-color:white;color:black;")
                                ),
                                Tr(
                                    Td(B(
                                        "total", style="background-color:white;color:black;")),
                                    Td(B("$"+self.get_all_price(book_id),
                                       style="background-color:white;color:black;"))
                                ),
                                Tr(Form(
                                    Button(
                                        "Pay", style="background-color:white;color:black;border: 2px solid black;border-radius: 8px;"),
                                    method="post",
    ...
    def get_html_price_summary(self, user_id, accom_id, form_data):
        ...
        accom_name = controlsystem.search_accomodation_by_id(
            accom_id).get_acc_name
        ...
        check_in_date = datetime.strptime(
            form_data.get("check-in"), "%Y-%m-%d")
        check_out_date = datetime.strptime(
            form_data.get("check-out"), "%Y-%m-%d")
        ...
        return Html(
            ...
            Body(
                ...
                Div(
                    Div(
                        Form(
                            Div(f"{accom_name}, {accom_address}",
                                cls="accom-name"),
    ...
    def create_coupon_to_user(self, booking_id):
        from .Booking import Booking
        from .User import Coupon
        booking_item = self.search_booking_by_id(booking_id)
        user = booking_item.get_member
        user.add_coupon(Coupon(12))
        user.add_coupon(Coupon(15))
        user.add_coupon(Coupon(20))

    def get_cou_list(self, booking_id):
        from .Booking import Booking
        from .User import Coupon
        from .User import Member
        booking_item = self.search_booking_by_id(booking_id)
        user = booking_item.get_member
        cou_list = user.get_coupons
        result = []
        for i in cou_list:
            code = i.get_code
            name = i.get_dis
            ...
            result.append(
                {"id": str(code), "name": f"ลดราคา {str(name)} % หมดอายุวันที่ {ex.strftime("%Y-%m-%d")}"})
        return result

    def get_dropdown_form(self, booking_id):
        cou_list = self.get_cou_list(booking_id)

        # เติม None ถ้าจำนวนไม่ถึง 5
        cou_list.extend([{"name": "ไม่มีคูปอง", "id": None}]
                        * (5 - len(cou_list)))

        return Div(
            H3("Coupon"), Form(
                Button("Add Coupon For Test", type="submit"),
                Input(type="hidden", name="booking_id", value=booking_id),
                method="post",
                action="/test_cou"),
            Form(
                Button("Del Cou", type="submit"),
                Input(type="hidden", name="booking_id", value=booking_id),
                Input(type="hidden", name="cou_code", value="del"),
                method="get",
                action="/coupon"),
            Form(
                *[
                    Button(
                        c["name"],
                        type="submit",
                        name="cou_code",  # กำหนด name ให้ส่งค่าไปที่เซิร์ฟเวอร์
                        value=c["id"] if c["id"] is not None else "",
                        disabled=c["id"] is None
                    )
                    for c in cou_list
                ],
                Input(type="hidden", value=booking_id, name="booking_id"),
                method="get", action="/coupon"
            ),
            style="background-color:white;"
        )

    def update_coupon(self, coupon, booking_id):
        booking_item = self.search_booking_by_id(booking_id)
        booking_item.add_coupon_by_id(coupon)

        return Redirect(f"/booking/{booking_id}")

    def create_coupon_test(self, booking_id):
        self.create_coupon_to_user(booking_id)
        return Redirect(f"/booking/{booking_id}")
